...
log_name = '{}_{}'.format('ADAM', now.strftime('%Y%m%d_%H%M%S'))
...
log_name = '{}_{}'.format('SGD', now.strftime('%Y%m%d_%H%M%S'))
writer = SummaryWriter('logs/{}'.format(log_name))
...
log_name = '{}_{}'.format('RMSprop', now.strftime('%Y%m%d_%H%M%S'))
writer = SummaryWriter('logs/{}'.format(log_name))
# model = Net(channels=3, im_size=32, num_classes=num_classes).to('cuda')
model.apply(init_weights)
...
log_name = '{}_{}'.format('ADAdelta', now.strftime('%Y%m%d_%H%M%S'))
writer = SummaryWriter('logs/{}'.format(log_name))
# model = Net(channels=3, im_size=32, num_classes=num_classes).to('cuda')
model.apply(init_weights)
